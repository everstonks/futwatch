...
dia = soup.find("h2").get_text().upper() # .a.get("name").upper()
...
df = df_lista[0]
colunas = {'\u200c':'JOGO', '\u200c.1':'CAMPEONATO', '\u200c.2':'HORÁRIO', '\u200c.3':'ONDE PASSA?', '\u200c.4':'TEMPO REAL'}
...
x = df_html.replace('<thead>','<thead><tr><th colspan=5 id="thdia"><center>'+dia+'</center><small><i class="fa fa-twitter"></i> <i class="fa fa-instagram"></i> <i class="fa fa-youtube-play"></i> <i class="fa fa-facebook"> @goal</i></small>').replace('class="col_heading level0 col2"','class="col_heading level0 col2" style="padding-right:35px"').replace('<style type="text/css">','<head><meta charset="utf-8"><meta name="viewport" content="width-device, initial-scale=1"><link rel="preconnect" href="https://fonts.gstatic.com"><link href="https://fonts.googleapis.com/css2?family=Titillium+Web:wght@400;600&display=swap" rel="stylesheet"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"></head><style type="text/css">#thdia {background: #000000} table{font-family: Titillium Web, sans-serif; border-collapse: collapse; box-shadow: 0 0 20px rgba(0, 0, 0, 0.15)}')
